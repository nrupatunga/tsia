...
def compute_mtf_statistics(mtf):
    ...
    if len(mtf.shape) == 2:
        ...
        avg_self_transition_prob = np.diagonal(
            mtf, axis1=1, axis2=2).mean(axis=1)
        std_self_transition_prob = np.diagonal(
            mtf, axis1=1, axis2=2).std(axis=1)
        avg_next_transition_prob = np.diagonal(
            mtf, offset=1, axis1=1, axis2=2).mean(axis=1)
        std_next_transition_prob = np.diagonal(
            mtf, offset=1, axis1=1, axis2=2).std(axis=1)
...
def get_multivariate_mtf(timeseries_list,
    ...
    constant_signals = tags_stats[(
        tags_stats['max'] - tags_stats['min']) == 0].index
...
def discretize(timeseries, n_bins=MTF_N_BINS, strategy=MTF_STRATEGY,
    ...
    bin_edges = np.hstack((timeseries.values.min(),
                           bin_edges[0], _find_max(timeseries.values)))
    bin_edges[-1] = max(custom_th, bin_edges[-1])
